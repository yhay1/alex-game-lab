...
export function ProjectAiChat({ projectId, projectName, assets = [] }: { projectId: string; projectName: string; assets?: AssetReference[] }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch(`/api/projects/${projectId}/ai`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messages: next, assetReferences: selected.map(({ id }) => ({ id })) }) })
