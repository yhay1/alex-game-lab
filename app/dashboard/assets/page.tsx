...
export default async function AssetsPage() {
  ...
  return <GameLabShell name={name}><div className="max-w-5xl"><p className="font-mono text-xs uppercase tracking-[0.2em] text-primary">Library</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">Assets</h1><p className="mt-2 max-w-xl text-muted-foreground">A project-wide asset library. Open a workspace to upload and manage the files belonging to that game.</p><section className="mt-8 rounded-2xl border border-border bg-card p-6"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-secondary text-primary"><Images className="size-5" /></span><div><p className="text-2xl font-semibold">{total}</p><p className="text-sm text-muted-foreground">Uploaded assets</p></div></div></section>{projects.length === 0 ? <section className="mt-6 rounded-2xl border border-dashed border-border p-8 text-center"><h2 className="font-semibold">No projects yet</h2><p className="mt-2 text-sm text-muted-foreground">Create a project before adding assets.</p><Link href="/create" className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-primary">Create Game <ArrowRight className="size-4" /></Link></section> : <section className="mt-6 grid gap-4 md:grid-cols-2">{assetGroups.map(({ project, assets }) => <Link key={project.id} href={`/projects/${project.id}`} className="rounded-2xl border border-border bg-card p-5 transition-colors hover:border-primary/60"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">{project.name}</h2><ArrowRight className="size-4 text-muted-foreground" /></div><p className="mt-2 text-sm text-muted-foreground">{assets.length} {assets.length === 1 ? 'asset' : 'assets'} in this workspace</p></Link>)}</section>}</div></GameLabShell>
}
